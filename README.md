...
The complete run writes raw checkpoint metrics, head ablation results, a summary, and figures to `results/`.
...
- Causal stability: correlation of the head ablation profile with the final checkpoint at least 0.90.

These thresholds are declared before examining the result to reduce narrative flexibility. Results should be interpreted across seeds.

## Limitations

- Modular addition is a controlled algorithmic task, not language modeling.
- Because training covers the complete truth table, the model addresses task mastery rather than generalization.
